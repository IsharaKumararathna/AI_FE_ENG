...
| AI stage unit | `Aife.Ai.Tests` | Stages with mocked `LlmRouter` and `IKnowledgeProvider`. | Fast. |
| Knowledge unit | `Aife.Knowledge.Tests` | `JsonKnowledgeProvider` query logic against sample data. | Fast. |
| Contract | `Aife.Contracts.Tests` | JSON Schema validation of the three schemas and sample datasets. | Fast. |
| Integration | `Aife.Infrastructure.Tests`, `Aife.Api.Tests` | Cosmos repositories with emulator, API endpoints with test server. | Slower. |
...
- The three JSON Schemas (component catalog, intermediate UI, knowledge base) are
